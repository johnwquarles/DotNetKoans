...
namespace DotNetKoans.CSharp
{
    public class AboutStrings : Koan
    {
        ...
        public void AnotherWayToCreateAStringWhichContainsDoubleQuotes()
        {
            ...
            // # "the only exception being a quote-escape-sequence"; think this is that. ("" to escape a double quote)
            var str = @"Hello, ""World""";
            Assert.Equal(14, str.Length);
        }
...
broken line";
        ...
        public void StringsAreReallyImmutable()
        {
            ...
            // # ie, strA still exists in memory but has no reference point to it anymore (it belongs to the new strA).
        }
		...
		public void StringsCanBePaddedToTheLeft()
		{
            ...
            // # create a 3-character string, write from the end
		...
		{
            // # same here, but now the end is at -3 and is thus the left side of the string.
			var str = string.Format("{0,-3:}", "x");
			Assert.Equal("x  ", str);
		}
		...
		{
            // # see list at https://msdn.microsoft.com/en-us/library/8kb3ddd4(v=vs.110).aspx
        ...
        public void YouCanGetASubstringFromAString()
        {
            ...
            // from index 7, three characters (both numbers aren't indices, just the first).
